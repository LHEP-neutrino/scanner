...
Float_t *tile_plot(const char* rootfilename,const float mean_of_all_monitors){
	...
	TString strFullpath = TString("/home/daq/ArCLight_3DScan/scan_ACL3_series/scan_plots_ACL3/")+TString(string(rootfilename).substr(0,string(rootfilename).size()-5))+TString("/");
	...
	if(correction){
		...
		*/
		...
		gStyle->SetOptStat("");
		h_cor->Draw("colz");
		h_cor->SetMaximum(200);//set the maximum of the z axis
		...
		for(int i=0;i<6;i++){
			...
			h_c->Draw("colz");
			h_c->SetMaximum(90);//set the maximum of the z axis
