...
float n_y = 10;//number of measurements for x direction
float y_max = 360;//length in y direction
float n_x = 8;//number of measurements for y direction
...
Float_t *tile_plot(const char* rootfilename,const float mean_of_all_monitors){
	...
	TString strFullpath = TString("/home/lhep/ArCLight_3DScan/scan_ACLFSD_1/scan_plots_RnD_new_setup/")+TString(string(rootfilename).substr(0,string(rootfilename).size()-5))+TString("/");
