...
float tilesizex = 300;
float tilesizey = 500;
float n_y = 100;//100;//33;//25;//number of measurements for y direction; high res (5mm): 100; 2cm steps: 25; 1cm steps: 50
float y_max = tilesizey;//length in y direction
float n_x = 60;//60;//20//15;//number of measurements for x direction High res (5mm): 60; 2cm steps: 15; 1cm steps: 30
...
Float_t *tile_plot(const char* rootfilename,const float mean_of_all_monitors){
	...
	TString strFullpath = TString("/home/lhep/ArCLight_3DScan/scan_ACLFSD_1/scan_plots_new_setup1/")+TString(string(rootfilename).substr(0,string(rootfilename).size()-5))+TString("/");
