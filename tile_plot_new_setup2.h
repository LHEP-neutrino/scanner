...
Float_t *tile_plot(const char* rootfilename,const float mean_of_all_monitors){
	...
	TString strFullpath = TString("/home/lhep/ArCLight_3DScan/scan_ACLFSD_1/scan_plots_new_setup2/")+TString(string(rootfilename).substr(0,string(rootfilename).size()-5))+TString("/");
	...
	if(correction){
		...
		mon_d->cd();
		t_out->Draw(Form("LY_tot_mon_nph:Entry$:LY_tot_mon_nph_rms>>h_temp3%s",rootfilename));
		...
		*/
		...
		//Canvas Labeling
		TCanvas* Scan_cor = new TCanvas("Scan_cor"+TString(rootfilename),"Scan plot corrected for all tiles: "+TString(rootfilename),tilesizex*2.1,tilesizey*2.1);
		Scan_cor->cd();
	
		//create Histogram for the corrected values
		TH2F* h_cor = new TH2F("h_cor"+TString(rootfilename),"Scan corrected for all tiles "+TString(rootfilename),n_x,0,300,n_y,0,y_max);
		t_out->Project("h_cor"+TString(rootfilename),"y:x",Form("LY_tot_nph*%f*%f/(LY_tot_mon_nph)*(LY_tot_nph>0)",C,factor)); //y<300 so that the monitor is not shown
		//t_out->Project("h_cor"+TString(rootfilename),"y:x",Form("LY_tot_nph*%f*%f/(%f*Entry$+%f)",C,factor,k,A)); //y<300 so that the monitor is not shown
		...
		//Canvas Labeling
		TCanvas* Scan_cor_e = new TCanvas("Scan_cor_e"+TString(rootfilename),"Scan error plot corrected for all tiles: "+TString(rootfilename),tilesizex*2.1,tilesizey*2.1);
		Scan_cor_e->cd();
		//create Histogram for the error of the corrected values
		TH2F* h_cor_e = new TH2F("h_cor_e"+TString(rootfilename),"3D Scan of the error corrected for all tiles "+TString(rootfilename), n_x,0,300,n_y,0,y_max);
		
		
		//VERBESSERE HIER NOCH DEN FEHELER FÜR DEN MONITOR
		//
		t_out->Project("h_cor_e"+TString(rootfilename),"y:x",Form("LY_tot_nph_rms*%f*%f/(LY_tot_mon_nph)*(LY_tot_nph>0)",C,factor)); 
		//
		...
		//each SiPM plotted
		//Canvas Labeling
		TCanvas* Scan_s_c = new TCanvas("Scan_s_c"+TString(rootfilename),"Scan plot for each SiPM corrected for all tiles: "+TString(rootfilename),tilesizex*2.1,tilesizey*2.1);
		...
		for(int i=0;i<6;i++){
			Scan_s_c->cd(i+1);
			TH2F* h_c = new TH2F(Form("h_c%d%s",i,rootfilename),Form("3D Scan SiPMs corrected mean %f to %f: %s",C,mean_of_all_monitors,rootfilename), n_x,0,300,n_y,0,y_max);
			t_out->Project(Form("h_c%d%s",i,rootfilename),"y:x",Form("LY_nph[%d]*%f*%f/(LY_tot_mon_nph)*(LY_nph[%d]>0)",i,C,factor,i));
	...
	float_t x_ar[] = {10,30,50,70,90,110,130,150,170,190,210,230,250,270,290};
	int color_palette[] = {kRed, kBlue, kGreen, kMagenta, kOrange+7,  
		kCyan, kBlack, kGray+2, kPink+9, kSpring+5,  
		kTeal+4, kAzure+7, kViolet+1, kPink+3, kOrange-3,  
		kBlue+2, kGreen+3, kMagenta+2, kCyan+3, kRed-7  };  
	...
	for (Long64_t i = 0; i < nEntries; i++) {
		...
		if (LY[0]+LY[1]+LY[2]+LY[3]+LY[4]+LY[5] > 0) {
			Double_t correctedLY = (LY[0]*C*factor+LY[1]*C*factor+LY[2]*C*factor+LY[3]*C*factor+LY[4]*C*factor+LY[5]*C*factor) / LY_tot_mon_nph;
